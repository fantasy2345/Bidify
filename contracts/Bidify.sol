...
contract Bidify is ReentrancyGuard, Ownable, IERC165 {
  ...
  function bid(uint64 id, address marketplace, uint256 price) external payable nonReentrant {
    ...
    require( listing.price < price, "this price is not highest");
    require( price >= listing.price + listing.price / 20, "the price has to be higher than 5%");
    require( _balances[msg.sender] >= price, "your token is not as much as you bid");
    uint256 nextBid = price;
